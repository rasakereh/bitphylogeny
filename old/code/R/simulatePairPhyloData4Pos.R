...
mutmat<-matrix(rep(0,8*N),nrow=N,byrow=T)
...
for(i in 1:N) {
       ...
       if( i/N < mix[1] ) {
	     ind <- ceiling(runif(4)*P)
             mutmat[i,1:4] = ind
             mutmat[i,5:8] = p[ind,1]
 
	} else {  if  (i/N< sum(mix[1:2]) ){
          ind <- ceiling(runif(4)*P)
          mutmat[i,1:4] <- ind
          mutmat[i,5:8] <- p[ind,2]
        } else {
        ...
        if (i/N<sum(mix[1:6])){
          ind <- ceiling(runif(4)*P)
          mutmat[i,1:4] <- ind
          mutmat[i,5:8] <- p[ind,6]
          
        } else{
       
        ind <- ceiling(runif(4)*P)
        mutmat[i,1:4] <- ind
        mutmat[i,5:8] <- p[ind,7] }}}}}}
...
write.csv(mutmat, '../data/syn_mutmat_5000_50_4.csv', row.names = FALSE)
