...
construct_genotype <- function(num_snvs,mix){
  # Mannually construct the genotypes star clones phylogeny 
  # tree:
  #        clone1
  #          |
  #        clone2
  #          |   
  # clone3 clone4 clone5  clone6 clone7 clone8 clone9  clone10 clone11 clone12 clone13 clone14 clone15 clone16 clone17 clone18
...
mix<-c(0.3,0.22,0.03,0.03,0.03,0.03,0.03,0.03,0.03,0.03,0.03,0.03,0.03,0.03,0.03,0.03,0.03,0.03)
...
for(epsilon in epslist) {
	...
	write.csv(cbind(mutmat,rep(1:18,reads_in_clone)), paste('noisy_small_clones_star_', num_snvs, '_', 
	                        num_reads, '_', epsilon, '_mutmat.csv', sep=""), 
	          row.names = FALSE)

}

write.csv(clone, paste('noisy_small_clones_star_', num_snvs, '_',
