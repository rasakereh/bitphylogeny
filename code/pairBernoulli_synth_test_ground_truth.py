...
root = SNVBernoulli( dims=dims, mutcut=init_mutcut, initial_snvs=genotypes[0] )
c1 = SNVBernoulli( parent=root,dims=dims, mutcut=init_mutcut, initial_snvs=genotypes[1] )
root.add_child(c1)
c2 = SNVBernoulli( parent=c1, dims=dims, mutcut=init_mutcut, initial_snvs=genotypes[2] )
c1.add_child(c2)
c3 = SNVBernoulli( parent=c1, dims=dims, mutcut=init_mutcut, initial_snvs=genotypes[3] )
c1.add_child(c3)
c4 = SNVBernoulli( parent=c1, dims=dims, mutcut=init_mutcut, initial_snvs=genotypes[4] )
c1.add_child(c4)
c5 = SNVBernoulli( parent=c2, dims=dims, mutcut=init_mutcut, initial_snvs=genotypes[5] )
c2.add_child(c5)
c6 = SNVBernoulli( parent=c4, dims=dims, mutcut=init_mutcut, initial_snvs=genotypes[6] )
...
for iter in range(-burnin,num_samples):
    ...
    print "nomix: data tree fit, node-wise fit ",tssb.complete_data_log_likelihood_nomix()
    ...
    filename = 'treescripts/testgraph_pairBernoulli1_groundt62_.gdl'
    fh2 = open(filename,'w')
    ...
    if iter > 0 and argmax(cd_llh_traces[:iter+1]) == iter:
        ...
        filename_best = "bests/pairBernoulli-test_groundt_-%s-best.pkl" % (codename)
...
plt.savefig('figures/pairBernoulli_groundt_trace.pdf',format='pdf')
clf()
...
filename = 'treescripts/testgraph_pairBernoulli1_groundt_.gdl'
fh2 = open(filename,'w')
best_node_fit.print_graph_pairing(fh2)
fh2.close()

filename_best = "bests/pairBernoulli1_groundt_test.pkl" 
fh = open(filename_best, 'w')
cPickle.dump(best_node_fit, fh)
fh.close()

fn = "bests/pairBernoulli1_groundt_test.pkl"
fh = open(fn, 'r')
