...
class SNVBernoulliFull(Node):
    ...
    def __init__(self, parent=None, dims=1, tssb=None, initial_snvs=0, bbeta=0.6):
        super(SNVBernoulliFull, self).__init__(parent=parent, tssb=tssb)
       ...
       # counts     = sum(data[:,[2,3]], axis=0)
        ...
        def logpost(params):
            if any(params>=1) or any(params<=0):
                ...
                return llh
            if self.parent() is None:
                llh = sum(betapdfln(params, self.alpha_base**((self.init_mean-self.bbeta())/abs(self.init_mean-self.bbeta())), self.beta_base**((self.init_mean-self.bbeta())/abs(self.init_mean-self.bbeta()))))
            else:
            ...
            llh = llh + sum(data*log(params) + (1.0-data)*log(1.0-params))
