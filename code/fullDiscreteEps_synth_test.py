...
#reader = csv.DictReader(open('./data/full8syn_mutmat.csv'),
#                        delimiter=',')
...
root = SNVDiscreteEpsilon( dims=dims, epsilon=init_epsilon, initial_snvs=clonal )
...
for iter in range(-burnin,num_samples):
    ...
    filename = 'treescripts/testgraph_fullDiscrete3_.gdl'
    fh2 = open(filename,'w')
    ...
    if iter > 0 and argmax(cd_llh_traces[:iter+1]) == iter:
        ...
        filename_best = "bests/fullDiscrete-test3-%s-best.pkl" % (codename)
...
plt.savefig('figures/fullDiscrete3_trace.pdf',format='pdf')
clf()
...
filename = 'treescripts/testgraph_fullDiscrete3.gdl'
fh2 = open(filename,'w')
best_node_fit.print_graph_full(fh2)
fh2.close()

filename_best = "bests/fullDiscrete3_test.pkl" 
fh = open(filename_best, 'w')
cPickle.dump(best_node_fit, fh)
fh.close()

fn = "bests/fullDiscrete3_test.pkl"
fh = open(fn, 'r')
