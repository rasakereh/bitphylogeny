...
construct_genotype <- function(num_snvs,mix){
  ...
  clone <- cbind(clone1,clone2,clone3,clone4,clone5,clone6,clone7,clone8,clone9,clone10,clone11,clone12)
  return(clone)
}
...
mix<-c(0.3,0.1,0.18,0.07,0.13,0.05,0.04,0.03,0.03,0.03,0.02,0.02)
...
while(ncol(t(unique(t(clone))))<12) {
	clone = construct_genotype(num_snvs,snvprop)
}
...
for(epsilon in epslist) {
	...
	write.csv(cbind(mutmat,rep(1:12,reads_in_clone)), paste('noisy_small_clones_', num_snvs, '_', 
	                        num_reads, '_', epsilon, '_mutmat.csv', sep=""), 
	          row.names = FALSE)

}

write.csv(clone, paste('noisy_small_clones_', num_snvs, '_',
