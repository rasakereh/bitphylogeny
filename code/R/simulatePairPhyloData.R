...
mutmat<-matrix(rep(0,4*N),nrow=N,byrow=T)
...
for(i in 1:N) {
       ...
       if( i/N < mix[1] ) {
	     ind <- ceiling(runif(2)*P)
             mutmat[i,1:2] = ind
             mutmat[i,3:4] = p[ind,1]
 
	} else {  if  (i/N< sum(mix[1:2]) ){
          ind <- ceiling(runif(2)*P)
          mutmat[i,1:2] <- ind
          mutmat[i,3:4] <- p[ind,2]
        } else {
        ...
        if (i/N<sum(mix[1:6])){
          ind <- ceiling(runif(2)*P)
          mutmat[i,1:2] <- ind
          mutmat[i,3:4] <- p[ind,6]
          
        } else{
       
        ind <- ceiling(runif(2)*P)
        mutmat[i,1:2] <- ind
        mutmat[i,3:4] <- p[ind,7] }}}}}}
