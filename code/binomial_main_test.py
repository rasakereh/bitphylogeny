...
reader = csv.DictReader(open('./pyclone-data/SRR385941.tsv'),
                        delimiter='\t')
...
for row in reader:
    ...
    data.append([row['var_counts'],row['ref_counts']])
...
dims = 1
root = Binomial_test( dims=dims, drift=init_drift )
...
filename = 'testgraph_binomial.gdl'
fh2 = open(filename,'w')
...
numpy.savetxt("binomial_cell_freq.csv", cell_freq_traces, delimiter=",")
