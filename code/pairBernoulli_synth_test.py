...
for iter in range(-burnin,num_samples):
    ...
    if iter > 0 and argmax(cd_llh_traces[:iter+1]) == iter:
        ...
        filename_best = "bests/pairBernoulli-test-%s-best.pkl" % (codename)
...
plt.savefig('figures/pairBernoulli1.pdf',format='pdf')
clf()
...
filename = 'testgraph_pairBernoulli1.gdl'
fh2 = open(filename,'w')
best_node_fit.print_graph(fh2)
fh2.close()

filename_best = "bests/pairBernoulli1_test.pkl" 
fh = open(filename_best, 'w')
cPickle.dump(best_node_fit, fh)
fh.close()

fn = "bests/pairBernoulli1_test.pkl"
fh = open(fn, 'r')
